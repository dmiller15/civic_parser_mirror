...
for line in open(sys.argv[2]):
    ...
    if info["transcript"] == "None" or info["transcript"] == ".":
        f.write("\t".join([var, p_type, var_info[var], info["civic_var_id"], info["transvar_prot_gDNA"], info["transcript"], transvar_prot_transcript, candidate_codons, aliases, info["source"][:-1], "No_civic_info"]) + "\n")
    else:
        if transvar_prot_transcript in info["transcript"]:
            f.write("\t".join([var, p_type, var_info[var], info["civic_var_id"], info["transvar_prot_gDNA"], info["transcript"], transvar_prot_transcript, candidate_codons, aliases, info["source"][:-1], "Match"]) + "\n")
        elif "ENST" not in transvar_prot_transcript:
            f.write("\t".join([var, p_type, var_info[var], info["civic_var_id"], info["transvar_prot_gDNA"], info["transcript"], transvar_prot_transcript, candidate_codons, aliases, info["source"][:-1], "No_civic_info"]) + "\n")
        else:
            f.write("\t".join([var, p_type, var_info[var], info["civic_var_id"], info["transvar_prot_gDNA"], info["transcript"], transvar_prot_transcript, candidate_codons, aliases, info["source"][:-1], "Not_match"]) + "\n")
